...
function getBuddyListFriends(list: BuddyList): Friend[] {
	return list.members.reduce((cumulativeFriends, currentBuddy) => {
		if ("phone" in currentBuddy) {
			cumulativeFriends.push(currentBuddy);
		}
		return cumulativeFriends;
	}, [] as Friend[]);
}

console.log("Friends in football buddies list: ", getBuddyListFriends(myFootballBuddies));
console.log("Friends in band buddies list: ", getBuddyListFriends(myBandBuddies));
