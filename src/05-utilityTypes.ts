...
type IntersectingData = Pick<Friend & Colleague, "name" | "age" | "contact">;
function intersection(friends: Friend[], colleagues: Colleague[]): IntersectingData[] {
	let result: IntersectingData[] = [];
	friends.reduce((res, friend) => {
		const colleague = colleagues.find((col) => col.name === friend.name);
		if (colleague) {
			res.push({ name: friend.name, age: friend.age, contact: colleague.contact });
		}
		return res;
	}, result);
	return result;
